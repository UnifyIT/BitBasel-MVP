...
<?php $show_author = CustomizeSettings::get_setting('wb4wp_single_post_section_show_author_setting') ?>
<?php $show_author_avatar = CustomizeSettings::get_setting('wb4wp_single_post_section_show_author_image_setting') ?>
<?php $show_pub_date = CustomizeSettings::get_setting('wb4wp_single_post_section_show_publication_date_setting') ?>
<?php $show_social_sharing = CustomizeSettings::get_setting('wb4wp_single_post_section_show_social_sharing_setting') ?>
  ...
  <div class="wb4wp-author">
    ...
    <?php if($show_author_avatar): ?>
      <img 
        ...
        width="140"
        height="140"
        alt="profile picture of <?= the_author_meta( 'display_name' , $author_id ) ?>"
      >
    <?php endif ?>
    ...
    <div class="wb4wp-social">
      ...
      <?php 
        foreach ($social_channels as $account => $url):        
          ...
          if( !empty($share_url) ):
      ?>
        <a href="<?= $share_url ?>" class="wb4wp-social-icon fa fa-<?= $account ?>" aria-label="Go to our <?= $account ?> page" target="_blank">
        </a>
      <?php endif; ?>
      <?php endforeach; ?>
    </div>
